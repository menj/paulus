...
1. Upload `paulus-2.33.6.zip` under Appearance, Themes, Add New, and activate it.
...
| Publisher | Name, registration number, address and contact details |
...
paulus/
  ...
  assets/icons/   share marks from the Minimalist Social Icons pack (facebook, x, whatsapp, telegram), the theme's email, print and search marks, a full-colour PDF file mark (the only icon not drawn in the text colour), and social/ with the full pack (45 platforms) for the footer
...
| Shortcode | Output |
| --- | --- |
| `[paulus_hero]` | Front-page hero: kicker, heading, subheading, the author's name with "Author of" the book (from the Book tab), lede, buttons, portrait cutout |
...
| `[paulus_page_hero]` | Title panel for articles, pages and section pages; on articles the byline sets the author's name in Cinzel capitals as a signature, with "By" and the reading time in the label face, and the panel carries the share links (Facebook, X, WhatsApp, Telegram, email, plain links; marks from the Minimalist Social Icons pack, inlined by `paulus_icon()`, which accepts only plain slugs as icon names and never passes them through `sanitize_file_name()`, since that rewrites a name such as "pdf" to "unnamed-file.pdf") |
...
| `[paulus_article_nav]` | Full-width limestone band under the meander: previous and next in reading order, and the study-questions link |
...
| `[paulus_book]` | Book panel; `full="1"` adds publisher contact details and makes the title the page's h1 |
...
| `[paulus_colophon]` | Footer credit line |
